...
export async function createBuilding(
  ...
  const { data: building, error: buildingError } = await supabase
    ...
    .insert({
      ...
      staircase_count: (survey.floors || []).reduce((sum, f) => sum + (f.staircaseCount || 0), 0),
      lift_count: (survey.floors || []).reduce((sum, f) => sum + (f.liftCount || 0), 0),
      total_toilets: (survey.floors || []).reduce((sum, f) => sum + (f.totalToilets || 0), 0),
      usable_toilets: (survey.floors || []).reduce((sum, f) => sum + (f.usableToilets || 0), 0),
      unusable_toilets: (survey.floors || []).reduce((sum, f) => sum + (f.unusableToilets || 0), 0),
...
export function buildingToFormData(building: BuildingWithShops): BuildingSurvey {
  return {
    ...
    floors: ((building.floors as any[]) || []).map((f: any) => ({
      floorNumber: f.floorNumber,
      roofType: f.roofType,
      staircaseCount: f.staircaseCount ?? 0,
      liftCount: f.liftCount ?? 0,
      totalToilets: f.totalToilets ?? 0,
      usableToilets: f.usableToilets ?? 0,
      unusableToilets: f.unusableToilets ?? 0,
    })),
